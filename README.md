...
| 기능 | 설명 |
|------|------|
| 📄 대본 업로드 & 파싱 | 텍스트 업로드 후 등장인물·대사 자동 분리 |
| 🧠 감정 분류 | BERT 기반 7감정 분류 → 서보 동작 패턴 자동 매핑 |
| 🔊 TTS 음성 생성 | Azure Neural TTS + SSML 감정 스타일 적용 (Andrew / Ava Neural) |
| 🤖 로봇 제어 | WebSocket으로 RPi5에 오디오 + 감정 전송 → 서보 고갯짓 + I2S 재생 |
| 🎙️ 발음 평가 | Azure Pronunciation Assessment → 정확도·유창성·완성도 점수 + 단어별 피드백 |
| 📊 연습 기록 | 세션별 점수 저장 및 히스토리 조회 |
